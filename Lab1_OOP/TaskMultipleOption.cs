...
namespace Lab1_OOP
{
    public class TaskMultipleOption : Task
    {
        ...
        public TaskMultipleOption(string question, List<string> answer_options, List<string> answers, int assessment, int count_correct_answer)
        {
        ...
        public override double ask_questions()
        {
            ...
            for (int i = 0; i < answer_options.Count; i++)
            {
                for (int j = 0; j < answers.Count; j++)
                {
                    ...
                    {
                        letter_correct_answer.Add(lettersForTest[i].letter.ToString().ToUpper());
                    }
